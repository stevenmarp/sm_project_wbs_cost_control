...
class SmProjectWbs(models.Model):
    ...
    _sql_constraints = [
        ('project_code_unique', 'unique(project_id, code)', 'WBS code must be unique per project.'),
        ('progress_range', 'check(progress >= 0 and progress <= 100)', 'Progress must be between 0 and 100.'),
    ]
...
class SmProjectWbsDependency(models.Model):
    ...
    _sql_constraints = [
        ('wbs_dependency_unique', 'unique(wbs_id, depending_wbs_id)', 'Two WBS items can only have one dependency relation.'),
        ('wbs_dependency_not_self', 'check(wbs_id != depending_wbs_id)', 'WBS item cannot depend on itself.'),
    ]
